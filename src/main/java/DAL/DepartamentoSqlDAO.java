...
public class DepartamentoSqlDAO implements IDepartamentoDAO {
    ...
    private final RowMapper<Departamento> depMapper = rs ->
    ...
    public List<Departamento> getDepartamentos() {
        return db.select("SELECT * FROM Departamento", depMapper, (Object[]) null);
    }

    @Override
    public Departamento procurarPorSigla(String sigla) {
        ArrayList<Departamento> lista = db.select(
                "SELECT * FROM Departamento WHERE sigla=?", depMapper, sigla);
    ...
    public boolean eliminarDepartamento(String sigla) {
        return db.execute("DELETE FROM Departamento WHERE sigla=?", sigla) > 0;
    }
}
